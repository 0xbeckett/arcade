...
let shown=false;
for(let i=0;i<30000&&!shown;i++){const r=s.rows();const[sx,sy]=shipXY();held.left=held.right=held.up=held.down=false;if(sx>=0){let L=0,R=0,C=0;for(let y=Math.max(0,sy-5);y<sy;y++)for(let x=0;x<COLS;x++)if(r[y][x]==='↓'){if(x<sx)L+=6-(sy-y);if(x>sx)R+=6-(sy-y);if(Math.abs(x-sx)<=1)C+=6-(sy-y);}if(C>0||L!==R){if(L>R)held.right=true;else if(R>L)held.left=true;else held[sx<COLS/2?'right':'left']=true;}if(sy>13)held.up=true;}
if(r.some(x=>x.includes('GAME OVER')))reg.onInput('a',true);
reg.update(1000/60);reg.render(s);
let __bd=0;if(s.rows().some(x=>x.includes("▓▓▓▓▓")))__bd=1;if(__bd&&(shown||true)){for(let k=0;k<45;k++){held.left=held.right=false;reg.update(1000/60);reg.render(s);}console.log('== BOSS WAVE ==');console.log('+'+'-'.repeat(COLS)+'+');for(const row of s.rows())console.log('|'+row+'|');console.log('+'+'-'.repeat(COLS)+'+');shown=true;}}
if(!shown)console.log('boss not reached');
